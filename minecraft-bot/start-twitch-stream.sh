...
# Note: Simple text to avoid ffmpeg parsing issues
ffmpeg \
    -f lavfi -i color=c=black:s=${RESOLUTION}:r=${FPS} \
    -f lavfi -i anullsrc=r=44100:cl=mono \
    -vf "drawtext=text=Minecraft_AI_Bot_Training:x=(w-text_w)/2:y=(h-text_h)/2:fontsize=32:fontcolor=white:box=1:boxcolor=black@0.5" \
    -vcodec libx264 \
    -preset veryfast \
    -b:v ${BITRATE} \
    -maxrate ${BITRATE} \
